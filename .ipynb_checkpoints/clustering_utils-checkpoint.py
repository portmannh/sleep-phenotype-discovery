...
def plot_dbscan_panels(
    ...
    """Plot one row of scatter panels coloured by DBSCAN cluster labels.
    Parameters
    ----------
    results : list of (coords, labels, title)
    results : list of (coords, labels, title) or (coords, labels, title, cohort)
        ``cohort`` is an array-like of cohort codes per point (e.g. ``df['dataset']`` values).
    cohort_markers : dict, optional
        Marker style per cohort code, e.g. ``{'sc': 'x', 'st': 'o'}``.
    cohort_legend : dict, optional
        Legend text per cohort code, e.g. ``{'sc': 'sleep-cassette', 'st': 'sleep-telemetry'}``.
    """
    if cohort_markers is None:
        cohort_markers = {"sc": "x", "st": "o"}
    if cohort_legend is None:
        cohort_legend = {"sc": "sleep-cassette", "st": "sleep-telemetry"}
